...
const STRINGS = {
  ...
  ko: {
    ...
    attireTextPre: "칵테일 또는 가든파티 차림을 부탁드립니다.",
    attireTextPost: "나파 밸리 가을 저녁이 지닌 낭만에 당신의 우아함을 더해주세요.",
    attireList: [
    ...
    registryBody: "여러분이 저희의 결혼을 함께 축하해 주시는 것만으로도 가장 큰 선물입니다.\n조금 더 마음을 나누고 싶으시다면 신혼여행 기금을 이용해 주세요 :)",
    ...
    sendRsvp: "회신 보내기",
    footerHeadline: "만날 날을 손꼽아 기다리고 있어요!"
  }
};
...
function langFromLocation() {
  return "ko"; // this file drives the Korean page only
}
  ...
  return (
    <section id="story" data-screen-label="Our Story">
      <div className="container">
        ...
        <div className="story-body" style={{ fontSize: "18px", maxWidth: "100%", lineHeight: "1.35", color: "#3D3C3A", fontWeight: "300", width: "780px" }}>
          ...
          {rows.map((row, i) => {
            if (row.kind === "interlude") {
              ...
              <div className="tl-side" style={{ ...{ padding: row.side === "left" ? "0px 28px 0px 0px" : "0px 0px 0px 28px", textAlign: row.side === "left" ? "right" : "left" }, padding: "0px 0px 0px 20px" }}>
                  ...
                  <p className="tl-interlude-text" style={{ marginLeft: row.side === "left" ? "auto" : "0", marginRight: row.side === "left" ? "0" : "auto", color: "rgb(111, 111, 111)", fontSize: "17px", width: "284px", lineHeight: "19px" }}>{row.item.interlude}</p>
                </div>;
              return (
              ...
              <div key={i} className={`tl-row ${side}`} style={{ padding: "22px 0px" }}>
                ...
                <>
                    <div className="tl-side event" style={{ borderWidth: "0px", borderStyle: "solid", textAlign: "right", margin: "0px", padding: "0px 20px 0px 0px", width: "303px" }}>
                      <h3 className="tl-title" style={{ fontFamily: "\"PP Editorial New\"", fontWeight: "500", fontSize: "22px", margin: "0px 0px 7px" }}>{item.title}</h3>
                      ...
                      <p className="tl-desc" style={{ marginLeft: "auto", color: "rgb(111, 111, 111)", fontSize: "17px", margin: "0px", width: "286px", lineHeight: "19px" }}>{renderDesc(item.desc)}</p>
                    </div>
                    {illo}
                  </> :
                ...
                <>
                    ...
                    <div className="tl-side event" style={{ textAlign: "left", margin: "0px", padding: "0px 0px 0px 20px" }}>
                      ...
                      <p className="tl-desc" style={{ color: "rgb(111, 111, 111)", fontSize: "17px", margin: "0px", lineHeight: "19px" }}>{renderDesc(item.desc)}</p>
                    </div>
                  </>
                }
              </div>);
...
function HotelCard({ hotel }) {
  ...
  return (
    <Tag className="hotel-card" {...linkProps}>
      ...
      <div className="hotel-meta" style={{ letterSpacing: "0px", fontSize: "10.5px" }}>{t.hotelMeta(hotel.stars, hotel.distance)}</div>
      <p className="hotel-desc" style={{ fontSize: "17px", color: "rgb(111, 111, 111)", lineHeight: "19px" }}>{renderDesc(hotel.desc)}</p>
      {hotel.img ?
    ...
    <section id="travel" data-screen-label="Travel & Stay">
      <div className="container">
        ...
        <div className="travel-block" style={{ lineHeight: "1.3" }}>
          <p style={{ fontSize: "18px", lineHeight: "24px", color: "#3D3C3A" }}>{renderDesc(t.travelParagraph)}</p>
        </div>
  ...
  return (
    <div className="place-cat">
      ...
      <div key={i} className="place-item">
          ...
          <p className="place-desc" style={{ color: "rgb(111, 111, 111)", fontSize: "17px", lineHeight: "19px" }}>{p.desc}</p>
        </div>
      )}
    </div>);
  ...
  return (
    <section id="things" className="todo-bg" data-screen-label="Things to Do">
      <div className="container">
        ...
        <p style={{ textAlign: "center", fontFamily: "var(--f-serif)", color: "#3D3C3A", maxWidth: 720, margin: "0 auto 1.2em", fontSize: "18px", lineHeight: "24px", width: "704px" }}>{renderDesc(t.thingsIntro1)}</p>
        <p style={{ textAlign: "center", fontFamily: "var(--f-serif)", color: "#3D3C3A", maxWidth: 720, margin: "0 auto 56px", fontSize: "18px", lineHeight: "1.35", width: "540px" }}>{renderDesc(t.thingsIntro2)}</p>
    ...
    <section id="registry" className="registry-section" data-screen-label="Registry">
      <div className="container-narrow">
        ...
        <div className="accounts-block">
          <p className="accounts-intro">신혼여행 기금 외에 마음을 전하고자 하시는 분들을 위해<br className="desktop-br" /> 아래 계좌번호를 안내드립니다.</p>
          <div className="accounts-groups">
            <div className="accounts-group">
              <div className="accounts-group-label">신부측 혼주</div>
              <AccountRow name="정병억" role="신부 아버지" bank="신한은행" number="110-308-615423" />
              <AccountRow name="이정애" role="신부 어머니" bank="농협" number="218-12-016432" />
            </div>
            <div className="accounts-group">
              <div className="accounts-group-label">신부</div>
              <AccountRow name="정민경" bank="우리은행" number="1002-547-853714" />
            </div>
          </div>
        </div>
      </div>
...
function AccountRow({ name, role, bank, number }) {
  const [state, setState] = useState("idle");
  const flash = (s) => { setState(s); setTimeout(() => setState("idle"), 1800); };
  ...
  return (
    <div className="account-row">
      <div className="account-lines">
        <div className="account-name">{name}{role ? <span className="account-role"> ({role})</span> : null}</div>
        <div className="account-num" onClick={(e) => { try { const r = document.createRange(); r.selectNodeContents(e.currentTarget); const s = window.getSelection(); s.removeAllRanges(); s.addRange(r); } catch (err) {} }}>{bank}<span className="account-bank-gap">{number}</span></div>
      </div>
      <button type="button" className="account-copy" onClick={copy} aria-label={name + " 계좌번호 복사"}>
        {copied ?
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg> :
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="12" height="12" rx="2" /><path d="M5 15V5a2 2 0 0 1 2-2h10" /></svg>}
        <span className="account-copy-label">{copied ? "복사됨" : state === "failed" ? "직접 복사" : "복사"}</span>
      </button>
    </div>);
}
  ...
  return (
    <section id="faqs" data-screen-label="FAQs">
      <div className="container-narrow">
        ...
        <p style={{ textAlign: "center", fontStyle: "italic", maxWidth: 540, margin: "0 auto 56px", fontFamily: "\"PP Editorial New\"", fontWeight: "300", fontSize: "17.5px", lineHeight: "1.5", color: "rgb(0, 0, 0)" }}>{t.faqIntroPre}<br className="mobile-br" />{t.faqIntroPost}</p>
        ...
        <div className="faq-list">
          ...
          <div key={i} className={"faq-item" + (openSet.has(i) ? " open" : "")}>
              ...
              <div className="faq-body">
                <div className="faq-body-inner" style={{ fontSize: "17px", lineHeight: "23px", color: "#6F6F6F" }} dangerouslySetInnerHTML={{ __html: f.a }} />
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <section id="rsvp" className="rsvp-section" data-screen-label="RSVP">
      <div className="container-narrow">
        ...
        <p style={{ textAlign: "center", fontStyle: "italic", maxWidth: 600, margin: "0 auto 56px", lineHeight: 1.6, fontSize: "17.5px", fontWeight: "300", fontFamily: "\"PP Editorial New\"", color: "rgb(0, 0, 0)" }}>{t.rsvpIntroPre}<br className="mobile-br" />{t.rsvpIntroPost}</p>
...
Object.assign(window, {
  ...
  Registry, AccountRow, FAQs, RSVP, Footer, SectionHeader, SectionDisplayTitle, Monogram
});

// ─────────────────────────────────────────────────────────────
// Main App
// ─────────────────────────────────────────────────────────────
// Korean page content. Edit these strings directly.
const WEDDING_DATA_KO = {
  "schedule": [
    {
      "id": "reception",
      "time": "4:30 — 5:00 PM",
      "title": "리셉션",
      "desc": "환영합니다 :) 방명록과 즉석 카메라를\n{m}준비해 두었어요. 오늘의 특별한 순간을 담아주세요.",
      "loc": "The Vineyard"
    },
    {
      "id": "ceremony",
      "time": "5:00 — 5:30 PM",
      "title": "예식",
      "desc": "예식은 나파 밸리의 아름다움에\n둘러싸인 {m}포도밭 사이에서 진행됩니다.",
      "loc": "The Vineyard"
    },
    {
      "interlude": "예식이 끝난 뒤, 호텔 정원을 따라 약 5분 정도 걸어 The Social Courtyard로 이동해주세요."
    },
    {
      "id": "cocktail",
      "time": "5:30 — 6:30 PM",
  ...
  "yountville_food": [
    { "name": "Bouchon Bakery", "url": "https://thomaskeller.com/yountville-california/bouchon-bakery/bakery/", "desc": "토마스 켈러 셰프의 사랑받는 베이커리로, 그의 Bouchon Bistro 바로 옆에 있습니다. 크루아상과 라떼로 아침을 시작해 보세요!" },
    { "name": "Clementine", "url": "https://clementineyountville.com/", "desc": "캘리포니아 요리를 선보이는 곳으로, 저희 예식장인 The Estate Yountville 안에 자리하고 있어요. 햇살 가득한 점심이나 올리브 나무 아래에서의 여유로운 저녁에 더없이 좋은 곳입니다." },
    { "name": "The French Laundry", "url": "https://thomaskeller.com/yountville-california/french-laundry/general-information/", "desc": "세계에서 가장 유명한 레스토랑 중 하나로, 저희 예식장 바로 옆에 있습니다. 아는 분들은 아실 거예요. 예약은 60일 전에 열리고 금세 마감되니 미리 계획하세요." },
    { "name": "Bistro Jeanty", "url": "https://bistrojeanty.com/", "desc": "분위기까지 완벽한 정통 프렌치 비스트로입니다. Joe와 Mia가 예식장을 처음 방문했을 때 들렀던 곳인데, 기대를 저버리지 않았어요." },
    { "name": "Bottega", "url": "https://www.botteganapavalley.com/", "desc": "욘트빌 중심부에 자리한 따뜻한 분위기의 이탈리안 레스토랑으로 여유로운 저녁 식사를 즐기기 좋습니다." }
  ],
  "yountville_todo": [
    { "name": "RH Yountville", "url": "https://rh.com/us/en/Yountville", "desc": "식사를 하지 않더라도 한 번쯤 들러볼 만한 곳입니다." },
    { "name": "Oakville Grocery", "url": "https://www.oakvillegrocery.com/?srsltid=AfmBOoq4KYdaV2VRF4X3GzBlHgy-3jekmF_Nn6YLCAr70TebEe2efdbF", "desc": "잠시 들러 구경하시고, 와인 또는 지역 특산물을 기념으로 구매해 보세요." }
  ],
  "napa_food": [
    { "name": "Morimoto Napa", "url": "https://morimotonapa.com/", "desc": "강변의 멋진 공간에서 즐기는 퓨전 감각의 일본 요리." },
    { "name": "Bistro Don Giovanni", "url": "https://www.bistrodongiovanni.com/", "desc": "큰길가에 자리한, 믿고 찾는 사랑받는 이탈리안 레스토랑. 현지인들이 즐겨 찾는 데는 이유가 있어요." }
  ],
  "napa_todo": [
  ...
  "faqs": [
    ...
    { "q": "예식에는 몇 시까지 도착해야 하나요?", "a": "오후 4시 30분까지 도착해 주세요. 하객 맞이는 오후 4시 30분부터 5시까지 진행되며, 예식은 오후 5시에 시작됩니다." },
    { "q": "예식은 야외에서 진행되나요?", "a": "네! 예식은 The Vineyard의 포도밭 사이 야외에서 진행됩니다. 욘트빌의 10월 저녁은 약 10°C(50°F)까지 내려갈 수 있으니, 편안하게 머무르실 수 있도록 가벼운 숄이나 재킷을 준비하시길 권해 드려요." },
    { "q": "비가 오면 어떻게 되나요?", "a": "걱정하지 않으셔도 됩니다. 실내 대체 공간이 준비되어 있어, 어떤 날씨에도 축하는 계속됩니다!" },
    { "q": "아이들도 초대되나요?", "a": "사랑스러운 아이들을 무척 아끼지만, 이번 자리는 성인 하객분들을 위한 축하의 자리로 진행하기로 했습니다. 아이들을 맡기실 수 있도록 준비하시어, 저희와 함께 편안한 저녁 시간을 보내시길 바랍니다!" },
    { "q": "예식 중에 사진을 찍어도 되나요?", "a": "물론이죠! 휴대폰이나 준비된 즉석 카메라로 원하시는 만큼 마음껏 순간을 담아 주세요. 여러분이 담아낸 모습이 무척 기대됩니다." },
    { "q": "채식이나 글루텐 프리 옵션이 있나요?", "a": "물론입니다. 다양한 식이 요구에 맞춰 여러 옵션을 준비할 예정입니다. 특별한 요청 사항이 있으시면 RSVP 양식을 통해 미리 알려주세요." }
  ]
};
const WEDDING_DATA_EN = WEDDING_DATA_KO;
